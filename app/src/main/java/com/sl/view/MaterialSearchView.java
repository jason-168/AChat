 ...
 */
public class MaterialSearchView extends LinearLayoutCompat /*implements Filter.FilterListener*/ {
    public static final int REQUEST_VOICE = 9999;

    private ArrayList<HashMap<String, Object>> searchDataList = new ArrayList<HashMap<String, Object>>();
...
//                attrs, R.styleable.MaterialSearchView, defStyleAttr, 0);
...
//    }
    ...
    private void onQueryItemClick(int res){
        CharSequence query = mSearchSrcTextView.getText();
        if (query != null && TextUtils.getTrimmedLength(query) > 0) {
            if (mOnQueryChangeListener == null || !mOnQueryChangeListener.onQueryItemSubmit(res,query.toString())) {
                closeSearch();
                mSearchSrcTextView.setText(null);
            }
        }
...
//        startFilter(mSearchSrcTextView.getText());
    ...
    private void setAdapter(SimpleAdapter adapter){
        mAdapter = adapter;
        mSuggestionsListView.setAdapter(adapter);
    }
...
//        }
    ...
    public void setSearchTypes(@IdRes int[] res){
        if(res != null && res.length > 0){
            ...
            for(int i=0; i<res.length; i++){
                ...
                m.put("icon", res[i]);
                m.put("text", "");
                searchDataList.add(m);
            }

            SimpleAdapter adapter = new SimpleAdapter(mContext, searchDataList, R.layout.search_suggest_item,
                    new String[]{"icon", "text"}, new int[]{android.R.id.icon, android.R.id.text1});
            setAdapter(adapter);

            setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    HashMap<String, Object> m = searchDataList.get(position);
                    onQueryItemClick((Integer)m.get("icon"));
                }
            });
        }else {
            mTintView.setVisibility(GONE);
    ...
    private void notifySearchTextChanged(CharSequence s){
        if(mAdapter != null) {
            for (int i = 0; i < searchDataList.size(); i++) {
                HashMap<String, Object> m = searchDataList.get(i);
                m.put("text", s.toString());
            }
...
//    }
    ...
    public interface OnQueryTextListener {
        ...
        boolean onQueryItemSubmit(int res, String query);
    }
